...
# 2. Create factory session class for handling async transaction blocks
...
# 3. Base declarative registry class
Base = declarative_base()

# 4. Async dependency to safely open and close sessions per request
async def get_db():
...
# 5. Async table initializer for application lifespan with resilient retry
async def init_db(max_retries: int = 10, delay: float = 2.0):
    for attempt in range(1, max_retries + 1):
        try:
            async with engine.begin() as conn:
                await conn.run_sync(Base.metadata.create_all)
            print(" [DB INIT] Auth service database tables initialized successfully.")
            return
